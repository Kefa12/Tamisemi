			...
			<div id="header" style="background-color:white;">
				...
				<div id="navbar1">
								<nav  style="background-color:4B7BE5; padding-left:0px; margin-left:95px;">
									<ul id="menu">
									<li><a href="{{ URL('dashboard') }}" class="ml-4 text-sm text-gray-700 dark:text-gray-500 underline">Home</a></li>
									  <li><a href="{{ URL('regionals') }}" class="ml-4 text-sm text-gray-700 dark:text-gray-500 underline">See Transfer Request</a></li>			
					
										<div id="navbar2">
										<li style="float: right;" width=100px>
										<select id="one">
									<figure class="user_avatar"><img src="../img/admin_profile.png" width="100%">
										<option value="two"><strong>({{ __('Regional_Director') }}) </strong></option>
        ...
        @foreach ($regionals as $regional)
            ...
            @if($regional->RD==Auth::user()->name&& $letter->regional==$regional->name)
               ...
               @elseif( $letter->WEO == 'APPROVED BY WEO')
                ...
                @endif
                @if( $letter->DEO == 'REJECTED BY DEO')
                <td><div class="col-xs-12 col-sm-12 col-md-12 text-center">
                <button type="submit" class="btn btn-danger"> REJECTED</button>
                </div></td>
               @elseif( $letter->DEO == 'APPROVED BY DEO ')
                 <td><div class="col-xs-12 col-sm-12 col-md-12 text-center">
                 <button type="submit" class="btn btn-primary">APPROVED</button>
                </div></td>
                @else
                ...
                @endif
                @if( $letter->DED == 'REJECTED BY DED')
                <td><div class="col-xs-12 col-sm-12 col-md-12 text-center">
                <button type="submit" class="btn btn-danger"> REJECTED</button>
                </div></td>
               @elseif( $letter->DED == 'APPROVED BY DED')
                 <td><div class="col-xs-12 col-sm-12 col-md-12 text-center">
                 <button type="submit" class="btn btn-primary">APPROVED</button>
                </div></td>
                @else
