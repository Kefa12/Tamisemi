    ...
    <div id="wrapper">
        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <a class="sidebar-brand d-flex align-items-center justify-content-center" href="#">
                ...
                <div class="sidebar-brand-text mx-3">District_Doctor</div>
            </a>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <!-- Sale & Revenue Start -->
              ...
              @foreach($letters as $letter)
                @if(auth::user()->district==$letter->cdistrict &&  $letter->chospital!='pending')
                  <input type="hidden" value="{{$data++}}"></input>
                @endif
              @endforeach
              @foreach($letters as $letter)
                @if((auth::user()->district==$letter->district) && ($letter->DEO=="pending"))
                  <input type="hidden" value="{{$k++}}"></input>
                @endif
              @endforeach
              @foreach($letters as $letter)
                @if((auth::user()->district==$letter->district) && ($letter->author!="unknown"))
                  <input type="hidden" value="{{$i++}}"></input>
                @endif
              @endforeach
              @foreach($chances as $chance)
                @if(auth::user()->district==$chance->district && $chance->school=='pending')
