        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <!-- Sale & Revenue Start -->
              ...
              @foreach($letters as $letter)
                @if(auth::user()->district==$letter->cdistrict)
                  <input type="hidden" value="{{$data++}}"></input>
                @endif
              @endforeach
              @foreach($letters as $letter)
                @if((auth::user()->district==$letter->cdistrict) && ($letter->DED=="pending"))
                  <input type="hidden" value="{{$k++}}"></input>
                @endif
              @endforeach
              @foreach($letters as $letter)
                @if((auth::user()->district==$letter->cdistrict) && ($letter->author!="unknown"))
                  <input type="hidden" value="{{$i++}}"></input>
                @endif
              @endforeach
              @foreach($chances as $chance)
                @if(auth::user()->district==$chance->district)
